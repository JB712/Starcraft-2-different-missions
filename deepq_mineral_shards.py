...
def learn(env,
          ...
          batch_size=32,
          print_freq=1,
          checkpoint_freq=1000, #init=10000
	...
	'''
	Essai: continuer sur le fichier de training existant

	if os.path.exists("mineral_shards.pkl"):
	  print("Fichier trouve ! One le load")
	  train, update_target, debug = deepq.build_train(
	  ...
	  act = ActWrapper.load("mineral_shards.pkl", act_params=act_params)
	...
	with tempfile.TemporaryDirectory() as td:
		...
		for t in range(max_timesteps):
			if t%1000==0:
				print("C'est le test des ", t," steps")
			if t%10000==0:
			...
			if (checkpoint_freq is not None and t > learning_starts and
			    num_episodes > 10 and t % checkpoint_freq == 0):  #ini: num_episodes >100
				print("Doing a checkpoint every",checkpoint_freq,"! at step ", t)
